...
parser.add_argument("-n", action="store_true", help="[-n] Use 'Newline' instead of 'Space' Word Separator.  Only affects OUTPUT files because LA-list will accept input files with words separated by either newline(s) or space(s) and tab(s).  LA-list can use binary file input to strip words and $pec^a\\ Ch@racter5.")
